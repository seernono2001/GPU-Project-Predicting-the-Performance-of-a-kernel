...
REPETITION = 2 # SMALL NUMBER FOR TESTING (to mitigate noise)

# VERIFICATION THAT ALL WORKS
problem_sizes = [1000, 5000]
...
# COMBINATIONS AND RUNNING EXPERIMENTS
for ps in problem_sizes:
    for tpb in threads_per_block:
        for nb in num_blocks:
            for op in ops:
                
                # CAN DELETE THIS PRINT LATER (to see process) <------------------------
                print(f"Experiment: {EXECUTABLE} {ps} {tpb} {nb} {op}")
                
                # LISTS TO COLLECT RESULTS
                single_thread_times = [] # all single thread times
                kernel_times = [] # all kernel times
                speedups = [] # all speedups

                # REPEAT FOR AVERAGING
                for _ in range(REPETITION):
                    # RUN THE CUDA PROGRAM FROM INSIDE PYTHON ON THE COMMAND LINE
                    result = subprocess.run(f"{EXECUTABLE} {ps} {tpb} {nb} {op}",
                ...
                if single_thread_times:
                    ...
                    with open(CSV_FILE, "a", newline="") as f:
                        csv.writer(f).writerow([GPU,
                                                ps,
                                                tpb,
                                                nb,
                                                op,
                                                avg_single,
                                                avg_kernel, avg_speed])
